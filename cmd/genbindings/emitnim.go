...
func (gfs *nimFileState) emitParameterNim2CABIForwarding(p CppParameter, copy bool) (preamble, rvalue string) {
	...
	if p.ParameterType == "QString" || p.ParameterType == "QAnyStringView" {
		if copy {
			...
			rvalue = "struct_seaqt_string(data: " + nameprefix + "_copy, len: csize_t(len(" + p.nimParameterName() + ")))"
		} else {
			rvalue = "struct_seaqt_string(data: if len(" + p.nimParameterName() + ") > 0: addr " + p.nimParameterName() + "[0] else: nil, len: csize_t(len(" + p.nimParameterName() + ")))"
		}
		...
		rvalue = "struct_seaqt_array(len: csize_t(len(" + p.nimParameterName() + ")), data: if len(" + p.nimParameterName() + ") == 0: nil else: addr(" + nameprefix + "_CArray[0]))"
		...
		rvalue = "struct_seaqt_map(len: csize_t(len(" + p.nimParameterName() + ")),keys: if len(" + p.nimParameterName() + ") == 0: nil else: addr(" + nameprefix + "_Keys_CArray[0]), values: if len(" + p.nimParameterName() + ") == 0: nil else: addr(" + nameprefix + "_Values_CArray[0]),)"
		...
		rvalue = "struct_seaqt_map(len: 1,keys: addr(" + nameprefix + "_CArray_First),values: addr(" + nameprefix + "_CArray_Second),)"
...
func (gfs *nimFileState) emitCabiToNim(assignExpr string, rt CppParameter, rvalue string) string {
	...
	if rt.Void() {
		...
		// in both cases
		// This is not a seaqt_string and therefore MIQT did not allocate it,
		...
		// We receive the CABI type of a seaqt_string. Convert it into []byte
		// We must free the seaqt_string data pointer - this is a data copy,
		// not an alias
		...
		if !ok {
			panic("emitCabiToNim: Encountered an unknown Qt class " + rt.ParameterType)
		}
